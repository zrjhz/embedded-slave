...
官方循迹
*********/
// uint8_t _DCMotor::CarTrack(uint8_t Car_Spend)
// {
// 	unsigned long t;
...
/************************************************************************************************************
【函 数 名】：  CarTrack									循迹(自带屏蔽白卡，和冲特殊地形)
【参数说明】：	无
【返 回 值】：	1:表示十字路口特殊地形，路径自动+1，但是没做好，码盘值不对
【简    例】：	CarTrack(50),								以五十的速度循迹
************************************************************************************************************/
uint8_t _DCMotor::CarTrack(uint8_t Car_Spend)
{
	unsigned long t;
	uint8_t mode, _mode = 10, res = 0, timrout_cross = 0;
	uint8_t tp;
	uint8_t gd, fd, tgd, count1 = 0, count2 = 0, eigth_track, siven_track;
	uint8_t LSpeed, RSpeed;
	uint8_t firstbit[8];
	uint8_t RFID_FLA = 0, Space_TrackE, Space_TrackS;
	uint16_t CarState = 0;
	uint16_t Code_Value = 0, Space_Flag = 0;

	Space_TrackS = ExtSRAMInterface.ExMem_Read(TRACK_ADDR + 1);
	Space_TrackE = ExtSRAMInterface.ExMem_Read(TRACK_ADDR);
	CarState = Cumulate_WORB_FirstNumber(Space_TrackE, Space_TrackS);
	if (CarState != 6)
	{
		Back(50, 185);
		delay(500);
	...
	while (true)
	{
		...
		gd = ExtSRAMInterface.ExMem_Read(TRACK_ADDR);
		eigth_track = gd;
		fd = ExtSRAMInterface.ExMem_Read(TRACK_ADDR + 1);
		siven_track = fd;
		count1 = Count_Number(gd, 8);
		count2 = Count_Number(fd, 7);
		if (count1 != 0 && count2 != 0) // 有黑色区域
		{
			for (size_t i = 0x01; i < 0x100; i <<= 1)
			{
				if ((gd & i) == 0)
		...
		else // 白卡或者特殊地形,第一次出现白色区域
		{
			...
			while (1)
			{
				Space_TrackS = ExtSRAMInterface.ExMem_Read(TRACK_ADDR + 1);
				Space_TrackE = ExtSRAMInterface.ExMem_Read(TRACK_ADDR);
				if (Space_TrackS != 0x7f && Space_TrackE != 0xff)
				{
					Stop();
					break;
				}
			...
			if (CarState != 6)
			{
				...
				Stop();
				delay(500);
				Code_Value += uint16_t(ExtSRAMInterface.ExMem_Read(BASEADDRESS + CODEOFFSET) + (ExtSRAMInterface.ExMem_Read(BASEADDRESS + CODEOFFSET + 1) << 8));
				Code_Value += 100;
				Space_TrackS = ExtSRAMInterface.ExMem_Read(TRACK_ADDR + 1);
				Space_TrackE = ExtSRAMInterface.ExMem_Read(TRACK_ADDR);
				CarState = Cumulate_WORB_FirstNumber(Space_TrackE, Space_TrackS);
